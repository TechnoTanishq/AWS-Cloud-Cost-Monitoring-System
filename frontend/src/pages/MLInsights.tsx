      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            AI-powered cost predictions and anomaly detection.
          </p>
        </div>
            ...
            <>
              ...
              <div className="card-elevated p-5">
                ...
                <p className="text-2xl font-bold">
                  ${data?.predictedCost?.toLocaleString?.() ?? 0}
                </p>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  {data?.explanation ?? "No insights available"}
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={data?.forecast ?? []}>
                ...
                <Area dataKey="upper" stroke="none" fill="hsl(220,72%,50%)" fillOpacity={0.05} />
                <Area dataKey="lower" stroke="none" fill="hsl(220,72%,50%)" fillOpacity={0.05} />
                <Line type="monotone" dataKey="actual" stroke="hsl(220,72%,50%)" strokeWidth={2} dot={{ r: 2 }} />
                <Line type="monotone" dataKey="predicted" stroke="hsl(200,80%,50%)" strokeWidth={2} strokeDasharray="5 5" dot={false} />
              </ComposedChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        {/* Top Drivers */}
        {!loading && data?.topDrivers?.length > 0 && (
          <div className="card-elevated p-5">
                ...
                <div key={i} className="flex items-center gap-4 p-3 rounded-lg bg-destructive/5 border border-destructive/10">
                  <AlertTriangle
                    className={`h-4 w-4 shrink-0 ${
                      a.severity === "high" ? "text-destructive" : "text-chart-4"
                    }`}
                  />
                  <div className="flex-1">
                    <p className="text-sm font-medium">
                      {a.service} — {a.deviation ?? 0}% above expected
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Actual: ${a.actual ?? 0} · Expected: ${a.expected ?? 0} · {a.date ?? "-"}
                    </p>
                  </div>
                  <span
